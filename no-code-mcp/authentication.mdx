...
description: "Managed MCP includes a full OAuth 2.0 server with PKCE support, enabling MCP clients like Cursor, Claude Desktop, and other AI tools to securely connect to your paywalled MCP servers."
---
...
Managed MCP supports **Dynamic Client Registration**, allowing MCP clients to automatically register themselves as OAuth clients. This means users can simply paste your proxy URL into their MCP client, and the client will handle all OAuth setup automatically.
...
No additional configuration is needed—the login page automatically uses your provider branding configured in **Settings > Hosted pages**.
...
Managed MCP fully supports these custom protocols, ensuring seamless authentication across different MCP client applications. The OAuth server correctly handles redirects to these custom schemes.
...
- `claude.ai` — Claude.ai hosted MCP clients
- `chatgpt.com`, `chat.openai.com`, and `*.openai.com` — ChatGPT and OpenAI-hosted MCP clients
- `www.cursor.com` — Cursor Cloud Agents and Automations (`https://www.cursor.com/agents/mcp/oauth/callback`). Cursor desktop uses `cursor://` or `http://localhost:8787/callback` instead
- `*.mcpjam.com` — MCP Jam's hosted inspector subdomains
- `*.solvapay.com` — SolvaPay-hosted redirect endpoints
- Custom protocol schemes: `cursor://`, `claude://`, `vscode://`, and the other MCP-client schemes above
