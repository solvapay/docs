...
These helpers only decode claims (`customerRef`, `customer_ref`, `sub`) and do not verify signatures.
...
The MCP adapter routes paywall gate outcomes through a typed
`formatGate` channel — the transport emits `isError: false`, a plain-string
...
Use `paywall.decide()` directly when you want full control over the paywall
...
Legacy consumers that still `try/catch` a `PaywallError` keep working —
...
- [Express.js Integration Guide](./express) - Learn HTTP framework integration
