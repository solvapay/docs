...
[`npx solvapay@latest init`](/sdks/typescript/setup/cli), then return here for account-level setup tasks.
...
2. Choose your sign-in method:
   ...
   - **Continue with email** - Use your email address (sign-in is passwordless via a one-time code)
...
After your first product is created, SolvaPay opens your Console.
...
- **`termsUrl`** / **`privacyUrl`** — linked from mandate and consent copy in `<MandateText>` and the hosted checkout, sign-in, and OAuth consent pages.
