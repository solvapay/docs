...
Every provider automatically gets a `requests` meter when created. This is the only meter definition seeded by default — it tracks all billable request activity and powers limit enforcement, usage billing, and analytics. MCP tool calls additionally record usage under per-tool `tool:{toolName}` meter names.
...
The `requests` meter uses `count` aggregation, which counts each usage record as one unit. SolvaPay also supports `sum` aggregation for meters that accumulate numeric values (e.g. tokens consumed). `requests` is the only seeded meter *definition*; MCP tool calls also record usage under `tool:{toolName}` meter names.
