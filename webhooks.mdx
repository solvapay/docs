...
<Info>
**Reactivation**: When a pending cancellation is undone via `reactivateRenewal`, a `purchase.updated` event fires with `cancelledAt` cleared and `autoRenew` restored to `true`.

**Plan switching**: When `activatePlan` is called with a different plan than the customer's current purchase, two events fire: `purchase.expired` for the old purchase and `purchase.created` for the new one.
</Info>
