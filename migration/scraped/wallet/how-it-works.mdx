...
```
 Top-Up          Reserve          Capture          Settle┌──────┐       ┌──────────┐     ┌──────────┐     ┌──────────┐│Stripe│──────▶│  Lock     │────▶│ Deduct   │────▶│ Provider ││charge│ tokens│  tokens   │ run │ actual   │ fee │ payout   │└──────┘       └──────────┘     └──────────┘     └──────────┘                    │                                   │                    ▼ (on failure)                      ▼               ┌──────────┐                     ┌──────────┐               │ Release  │                     │  Stripe  │               │ tokens   │                     │ Connect  │               └──────────┘                     └──────────┘
```

### 1. Top-Up[​](#1-top-up "Direct link to 1. Top-Up")
...
### 2. Reserve (Verify)[​](#2-reserve-verify "Direct link to 2. Reserve (Verify)")
...
### 3. Capture (Settle)[​](#3-capture-settle "Direct link to 3. Capture (Settle)")
...
### 4. Release[​](#4-release "Direct link to 4. Release")
...
## Immutable Ledger[​](#immutable-ledger "Direct link to Immutable Ledger")
...
### Lock States[​](#lock-states "Direct link to Lock States")

```
              settlereserved ─────────────▶ settled    │    ├── release ──────▶ released    │    └── TTL expiry ──▶ expired (auto-released)
```
...
## Balance Caching[​](#balance-caching "Direct link to Balance Caching")
