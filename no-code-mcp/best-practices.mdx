...
| **On exceed** | `limit.onExceed` | `block` stops access; `top_up` keeps serving and pays overage from prepaid credits at the per-request rate |
...
2. **Overage** (`onExceed: "top_up"`) — keep serving and pay usage beyond the included cap from prepaid credits at the per-request rate. The request is denied when the balance cannot cover one more
3. **Unlimited** (`cap: 0`) — no included cap
4. **Pay-as-you-go** — omit `limit`; every request is paid from prepaid credits

### Included recommendations

- **Free tier:** Hard cap to prevent abuse (`limit.cap: 100`, `onExceed: "block"`)
- **Paid tiers:** Usage-based with generous included usage, or hybrid with overage (`onExceed: "top_up"`)
- **Enterprise:** `limit.cap: 0` (unlimited) or a very high cap
- **Unlimited free tier:** `limit.cap: 0` means unlimited, not "zero included". That is still a plan-derived included cap. A per-customer tracked cap on an otherwise-free *tool* is the TypeScript SDK `registerFree` path — see [Add a free allowance to MCP tools](/sdks/typescript/guides/free-allowance)
...
A Managed MCP "No plan" tool is public, untracked, and unlimited. It is not a per-customer free allowance. If you need a tracked cap that converts to a paid plan when exhausted, use the TypeScript SDK `registerFree` path — [Add a free allowance to MCP tools](/sdks/typescript/guides/free-allowance). There is no Console setting for that cap.
...
All hosted pages (login, checkout, account portal) automatically display your provider branding including logo, colors, and fonts. Configure these in **Settings > Hosted pages**. For advanced customization beyond branding, consider the SDK integration approach.
