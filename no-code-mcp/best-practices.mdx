...
4. On each request, SolvaPay queries the meter timeseries to check current usage against the plan's included cap

### Included settings

| Setting | Plan field | Description |
|---------|-----------|-------------|
| **Hard cap** | `limit` | Maximum requests per billing period (`0` = unlimited) |
| **Included** | `freeUnits` | Requests included before overage. `0` is the unlimited sentinel (`LimitOption.cap === 0`) |
| **Credits per request** | `creditsPerUnit` | Credits deducted per request for prepaid usage |
| **Overage policy** | `overagePolicy` | Whether to allow usage beyond the included cap (hybrid plans) |

See [Plans and billing glossary](/plans/glossary) for the credit peg, meter noun, and `freeUnits: 0` sentinel.

### Overage options

1. **Hard limit** (`limit` with no overage policy) - Block requests when included usage is exhausted. The paywall response includes a checkout URL.
2. **Soft limit** (hybrid plan with `overagePolicy.allowOverage: true`) - Allow continued use, charge overage at `overageRate` per request up to `maxOverage`
3. **Unlimited** (`limit: 0`) - No cap on usage

### Included recommendations

- **Free tier:** Hard limit to prevent abuse (e.g. `limit: 100`, `freeUnits: 100`)
- **Paid tiers:** Usage-based with generous included usage or hybrid with overage
...
- **Unlimited free tier option:** Set free-plan `freeUnits: 0` when you intentionally want unlimited
  free usage. That `0` is the unlimited sentinel, not "zero included".
