...
During provider onboarding, your first product is created in a guided flow.
...
Plans are configured during product creation. They are not created in a separate setup step.
...
Usage-based plans automatically measure requests and enforce limits. Hybrid plans combine a recurring base fee with usage-based overage tracking. See [Plans documentation](/plans/overview) for full details on each type.
...
Limit checks query the meter event timeseries in real time — `sum(meterName, customerRef, periodStart, now)` is compared against the plan's hard cap. When limits are exceeded, requests are denied with a paywall response that includes a checkout URL for upgrading.
