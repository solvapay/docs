...
For the framework-agnostic linkage model (`ensureCustomer(externalRef)` from your backend after your own IdP login), see [Customer linkage (bring your own client and IdP)](/sdks/typescript/guides/customer-linkage).
...
Use `@solvapay/next/middleware` on the server and `@solvapay/react/auth0` on the client. The canonical runnable reference is [`examples/nextjs-auth0`](https://github.com/solvapay/solvapay-sdk/tree/main/examples/nextjs-auth0).
...
export const proxy = createAuth0AuthMiddleware({
  auth0,
...
**Never** send Auth0 access tokens from the browser to SolvaPay APIs. SolvaPay billing calls use your `sk_*` secret server-side.
...
function App({ children }: { children: React.ReactNode }) {
  const { user } = useUser()
  const adapter = createAuth0ClientAuthAdapter({ userId: user?.sub })
...
- [Express.js Integration Guide](./express) - Use adapters with Express
- [Next.js Integration Guide](./nextjs) - Use adapters with Next.js
- [React Integration Guide](./react) - Use adapters with React
- [Webhooks](./webhooks) - Handle auth-related purchase events
