...
3. If no `meterId` or `limit` is `0` — **unlimited access**, allow immediately
4. **Resolve the meter name** from the `meterId`
5. **Query the timeseries** — `sum(providerId, meterName, customerId, periodStart, now)`
...
- [Meter Events](/meters/events) — how usage data is recorded
