...
SolvaPay emits **47 event types** spanning the full lifecycle — from customer and
...
| `purchase.plan_changed` | A subscription was upgraded or downgraded through checkout. A limit does not switch the plan. |
...
| `customer.credit.auto_topup_failed` | An automatic credit top-up charge failed. |
...
**Plan switching**: checkout that activates a different plan emits `purchase.expired`
for the old purchase, `purchase.created` for the new one, and `purchase.plan_changed`.
Hitting a usage cap does not emit `purchase.plan_changed`.
</Info>
...
SolvaPay only populates `product` when the customer is created through Managed MCP's
...
When you integrate with the TypeScript SDK (instead of Managed MCP's hosted OAuth),
...
```json
// customer.credit.auto_topup_failed — an off-session auto-recharge charge declined
{
  "type": "customer.credit.auto_topup_failed",
  "data": {
    "object": {
...
`paymentIntentId` is `null` when the charge was declined before a payment intent was
created. Use this event to notify the customer to update their payment method — after
repeated declines the auto-recharge config's status flips to `failed` and no further
charges are attempted until the card is updated.
...
`isManagedMcp` is the canonical Managed MCP flag. Payloads also include deprecated
`isMcpPay` with the same value for wire compatibility.
...
# Copy the HTTPS URL and add it as a webhook endpoint in the Console
```

### Sending a test event

Use the Console test button, or call the API directly:
