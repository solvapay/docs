...
Pair this with `createMcpOAuthBridge` from `@solvapay/server` to surface `customer_ref` on `extra.authInfo` — the core helpers read it from the synthesised request headers. A complete working server lives at [`examples/mcp-checkout-app/src/server.ts`](https://github.com/solvapay/solvapay-sdk/blob/main/examples/mcp-checkout-app/src/server.ts).
