---
title: "Tools"
description: "Complete reference for the 31 tools exposed by the SolvaPay MCP Server, organised across 7 domains."
---
...
A plan is an ordered `options[]` list. Recurring / usage-based / hybrid / one-time is a derived label — do not send `type`, `creditsPerUnit`, `basePrice`, `setupFee`, or `freeUnits`. See [Plans](/plans/overview).
...
| `options` | array | Yes | Composable pricing options (`charge`, `billingCycle`, `tier`, `limit`, `entitlement`, `trial`, `prepaid`, `discount`, `rollover`, `autoAssigned`, `hidden`) |
| `description` | string | No | Plan description |
| `currency` | string | No | ISO 4217 display currency (defaults from options) |
| `features` | object | No | Arbitrary feature flags |
| `metadata` | object | No | Arbitrary metadata |
| `status` | string | No | `active`, `inactive`, or `archived` |
| `maxActiveUsers` | number | No | Optional concurrent-user cap |
| `accessExpiryDays` | number | No | Optional access lifetime |
| `taxBehavior` | string | No | `auto`, `inclusive`, or `exclusive` |

Example `options` for a $19/month plan:
...
| `offset` | number | No | 0 | Pagination offset |
| `status` | string | No | — | Filter by status |
| `requiresPayment` | boolean | No | — | Filter by whether any charge amount is positive |
| `search` | string | No | — | Search by name |
...
| `name` | string | No | Updated name |
| `description` | string | No | Updated description |
| `currency` | string | No | Updated display currency |
| `options` | array | No | Replacement `options[]` (send the full list) |
| `features` | object | No | Updated feature flags |
| `metadata` | object | No | Updated metadata |
| `status` | string | No | Updated status |
| `maxActiveUsers` | number | No | Updated concurrent-user cap |
| `accessExpiryDays` | number | No | Updated access lifetime |
| `taxBehavior` | string | No | `auto`, `inclusive`, or `exclusive` |
