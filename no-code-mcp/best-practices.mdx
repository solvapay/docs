...
description: "This guide covers recommended patterns for configuring your no-code MCP servers, structuring plans, and handling common scenarios."
---
...
If your onboarding goal is frictionless adoption, you can start with an unlimited free tier by
setting `freeUnits: 0` on the free plan. This keeps access auth-protected while removing usage
...
3. Usage-based and hybrid plans automatically track requests and define a `limit`
...
| **Credits per unit** | `creditsPerUnit` | Credits per metered unit for usage-based billing |
...
- **Unlimited free tier option:** Set free-plan `freeUnits: 0` when you intentionally want unlimited
  free usage
...
- Store the API key in the no-code MCP integration settings (encrypted at rest)
- Never include it in client-facing URLs or responses
- Rotate keys periodically
- Use separate keys for the no-code MCP integration vs direct access
...
Regularly review the no-code MCP section of the SolvaPay Console to:
...
### Can I use the no-code MCP integration with any MCP server?
...
Any MCP client that supports OAuth 2.0 with PKCE can connect to no-code MCP servers. This includes Cursor, Claude Desktop, and other MCP-compatible tools. DCR support means clients can auto-configure themselves.
...
The no-code MCP section of the SolvaPay Console shows transaction history, usage statistics, and revenue metrics. You can also view per-product stats from the product detail page.
...
No. The no-code MCP integration provides fully hosted checkout and account management pages. Customers can purchase plans, manage their purchases, and view billing history without you building any UI. All pages are white-labeled with your branding.

### Can I migrate from the no-code path to SDK integration later?

Yes. You can start with the no-code MCP integration for quick setup and migrate to SDK integration if you need more control. Your plans, customers, and purchases remain intact—only the integration method changes.
...
- [No-code MCP integration overview](/no-code-mcp/overview) - Review core concepts
