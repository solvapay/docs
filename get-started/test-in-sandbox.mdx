...
description: "Use sandbox mode to test your integration before or after live activation."
---
...
If you are on the SDK path, run [`npx solvapay@latest init`](/sdks/typescript/setup/cli) first so your
...
You can switch between **production** and **sandbox** at any time. Many providers activate production first, then switch back to sandbox for safe testing and regression checks.
...
1. **Add your proxy URL to an MCP client**
   ...
   Copy the proxy URL from the Managed MCP section and add it to your MCP client (e.g., Cursor, Claude Desktop).
...
3. **Test tool access**
   ...
   Call a protected tool and verify it succeeds. Check the Console to see the usage recorded.
...
1. **Run the CLI setup**
   ...
   From your project root, run `npx solvapay@latest init` if you have not already.
...
- [ ] Users can authenticate (OAuth for Managed MCP, your auth for SDK)
