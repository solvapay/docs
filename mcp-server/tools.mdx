...
| `actionType` | string | No | Action type (`api_call` by default) |
| `units` | number | No | Usage units (default: `1`) |
| `outcome` | string | No | Outcome (`success`, `paywall`, `fail`) |
| `productReference` | string | No | Product reference |
| `purchaseReference` | string | No | Purchase reference |
| `metadata` | object | No | Arbitrary event metadata |
| `timestamp` | string | No | ISO 8601 timestamp |
| `idempotencyKey` | string | No | Idempotency key to deduplicate events |
