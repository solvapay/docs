---
title: "Plans"
description: "Plans define pricing models and access tiers for your products and billing strategy."
---
...
| `hidden` | `boolean` | When `true`, the plan is excluded from every customer-facing surface and can only be granted by direct assignment |
...
Mark a plan as hidden to keep it off every customer-facing surface — use this for enterprise or
negotiated deals that shouldn't appear publicly.

- Create hidden plans in the SolvaPay Console by enabling the hidden option on the plan form.
- Hidden plans are filtered out of the SDK catalog, checkout eligibility, and product responses.
  Customers cannot self-select them.
- Assign a hidden plan to a customer from the Console customer page. Direct assignment activates
  the plan without collecting payment and stamps the purchase with the `admin_assignment` origin —
  the only origin allowed to activate a hidden plan.
- Plans without the field remain visible; only `hidden: true` changes behavior.
...
- **Server default plan** — the `defaultPlanRef` on an MCP server determines which plan is used for general tool access
...
- [Handle webhooks](/guides/handle-webhooks) — purchase and payment lifecycle events
- [Usage events guide](/sdks/typescript/guides/usage-events) — record and query billable usage
