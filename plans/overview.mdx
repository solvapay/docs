...
| `onExceed` | `enum` | `block`, `throttle`, `charge`, `top_up`, or `auto_upgrade` |
...
A limit needs a `billingCycle`. Pay-as-you-go (no cycle) cannot carry included usage. A missing limit means every counted item is billed. It is not unlimited.

`onExceed` is what happens after the included count:

- `block` stops access
- `charge` bills overage on the saved card at the end of the billing period
- `top_up` draws prepaid credits for usage past the cap, and refuses the next request when the balance cannot cover one unit
- `throttle` keeps serving and tells the caller to slow down
- `auto_upgrade` switches the customer to `onExceedPricingId`
