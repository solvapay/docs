...
{
  "events": [
    { "meterName": "requests", "customerRef": "cus_abc123", "value": 1 },
    { "meterName": "requests", "customerRef": "cus_def456", "value": 1 }
  ]
}
```
...
Every MCP tool call through the SolvaPay proxy automatically records a usage record under a per-tool `tool:{toolName}` meter (e.g. `tool:get_customer`). No SDK integration needed — the proxy records usage with the outcome as a property:

```json
{
  "meterName": "tool:get_customer",
  "customerRef": "cus_abc123",
  "value": 1,
  "properties": {
    "outcome": "success"
...
Usage is paid from prepaid credits as each event is recorded, not billed at period end:

1. On pay-as-you-go, every counted item debits credits at the plan's rate (flat or tiered)
2. On a recurring plan, usage within the included cap is free
3. Past the cap (`onExceed: "top_up"`), or from the first item when the meter has no limit, each item debits credits
