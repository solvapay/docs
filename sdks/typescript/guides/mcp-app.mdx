...
Pair this with `createMcpOAuthBridge` from `@solvapay/mcp` to surface `customer_ref` on `extra.authInfo` — the core helpers read it from the synthesised request headers. For the full batteries-included setup use `createSolvaPayMcpServer` from `@solvapay/mcp-sdk`. A complete working server lives at [`examples/mcp-checkout-app/src/server.ts`](https://github.com/solvapay/solvapay-sdk/blob/main/examples/mcp-checkout-app/src/server.ts).
...
Behind the scenes the card calls `getPaymentMethod`, and `<UpdatePaymentMethodButton>` pre-fetches `createCustomerSession` so the portal link is ready the moment the user clicks.
