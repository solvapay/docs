...
| `limit.onExceed` | `block` stops access and offers the product's other plans at checkout; `top_up` draws prepaid credits past the cap |

Included usage needs a billing cycle. A pay-as-you-go plan cannot carry a limit.

Overage is usage beyond the included cap. `top_up` pays it from prepaid credits as each request is served, and denies the request when the balance cannot cover one unit. On a recurring plan with no limit, every counted item is paid from prepaid credits the same way. Usage is never billed to the card.
...
| `past_due` | A renewal, retry, or trial conversion could not be collected. Access continues while SolvaPay retries |
...
1. Charge the recurring flat `charge` off-session on the card saved at checkout, when `requiresPayment` is true
...
Recurring checkout saves the card for later renewals and failed-payment retries.
...
When a period ends, included usage resets with the new `periodStart`. Unused included usage does not carry over. Advancing `periodStart` resets visible usage without deleting usage events.
