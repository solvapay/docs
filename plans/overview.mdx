...
| `onExceed` | `enum` | `block` or `top_up` |
| `windowDays` | `number` | Required when `scope` is `rolling_window` |

A limit needs a `billingCycle`. Pay-as-you-go (no cycle) cannot carry included usage. A missing limit means every counted item is paid from prepaid credits. It is not unlimited.

`onExceed` is what happens after the included count:

- `block` stops access. The denial offers the product's other plans, and checkout opens on the plan the customer picks
- `top_up` draws prepaid credits for usage past the cap, and refuses the next request when the balance cannot cover one unit

Usage is never billed to the card. The API still accepts `onExceed: "charge"` and stores it as `top_up`.
...
| `onEnd` | `enum` | `convert` or `cancel` |

`convert` moves the customer onto the plan’s paid terms. `cancel` ends access. A reverse trial is `cancel` plus an auto-assigned free plan on the product, so the customer stays on the free tier when the trial ends.
...
`amountMinor: 2` is $0.02 per request. This plan has no billing cycle and no included cap. Do not send `creditsPerUnit` as a plan field.
...
Set `onExceed: "top_up"` so usage past the included count is paid from prepaid
credits rather than blocked.

Tiered usage is paid from the credit balance as each request is served, on a
recurring plan and on a plan with no billing cycle alike.
